			...
			>
				...
				>
					<button class="icon" data-state="closed"
						><div
							class="[&amp;_path]:stroke-current text-token-text-primary"
								...
								<svg
									...
									><defs
										><clipPath id="__lottie_element_82"
											><rect width="20" height="20" x="0" y="0"></rect></clipPath
										></defs
									><g clip-path="url(#__lottie_element_82)"
										><g transform="matrix(1,0,0,1,0,0)" opacity="1" style="display: block;"
											><g opacity="1" transform="matrix(1,0,0,1,2.75,3.5)"
												><path
				...
				>
					{#if folders[folderId]?.childrenIds}
						{@const children = folders[folderId]?.childrenIds
							.map((id) => folders[id])
							.sort((a, b) => {
								// Sort by last_chat_updated_at in descending order (most recent first)
								const aLastChat = a.last_chat_updated_at || a.updated_at || 0;
								const bLastChat = b.last_chat_updated_at || b.updated_at || 0;
