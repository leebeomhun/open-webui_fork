<script lang="ts">
	...
	const isKcdId = (id: string) =>
		id === 'server:8' || (typeof id === 'string' && (id.startsWith('server:8/') || id.startsWith('TOOL:server:8')));
	...
	function findKcdToolId(): string | undefined {
		...
		let candidate = available.find((id) => typeof id === 'string' && id.startsWith('TOOL:server:8'));
		if (!candidate) candidate = available.find((id) => typeof id === 'string' && id.startsWith('server:8/'));
